import { MaterialType, WebGLUniformHandler, PointLight, RyRenderCache, GLSL_F_BASIC, GLSL_F_PHONG, GLSL_V_BASIC, GLSL_V_PHONG, MAX_LIGHTS } from "../PrivateRyRender.js";
export class WebGLManager {
    constructor(gl) {
        this.vaoCache = {};
        this.shaderProgramCache = [];
        this.uniformCache = [];
        this.tex2dCache = {};
        this.texCubeCache = {};
        this.gl = gl;
    ...
    addVao(name, vertices, uv, normals, indices) {
    ...
    */
    addInstancedVao(name, vertices, uv, normals, indices, maxInstances) {
        if (this.vaoCache.hasOwnProperty(name)) {
            console.log(`VAO "${name}" is already loaded!`);
            return;
        }
    ...
    bindProgram(material) {
        ...
        uHandler.setIfExists("material.ambient" /* MTL_AMBIENT */, material.mtlAmbient);
        uHandler.setIfExists("material.diffuse" /* MTL_DIFFUSE */, material.mtlDiffuse);
        uHandler.setIfExists("material.specular" /* MTL_SPECULAR */, material.mtlSpecular);
        uHandler.setIfExists("material.shininess" /* MTL_SHINE */, material.mtlShininess);
        uHandler.setIfExists("texSample" /* TEX_SAMPLE */, material.texSample);
    ...
    setModelUniforms(model) {
        ...
        uHandler.setIfExists("model" /* MODEL */, model.worldTransforms.getArrayRef());
        if (uHandler.has("normal_matrix" /* NORM_MAT */)) {
            uHandler.set("normal_matrix" /* NORM_MAT */, model.getNormalMatrix());
        }
    }
    setCameraUniforms(view, proj, cameraPos) {
        for (const uHandler of this.uniformCache) {
            uHandler.setIfExists("view" /* VIEW */, view);
            uHandler.setIfExists("projection" /* PROJ */, proj);
            uHandler.setIfExists("cameraPos" /* CAM_POS */, cameraPos);
        }
    }
    ...
    addAttribute(data, location, valuesPerVertex) {
        var gl = this.gl;
