...
export const GLSL_V_BASIC = `
    ...
    void main(void) {
        gl_Position = ${"projection" /* PROJ */} * ${"view" /* VIEW */} * ${"model" /* MODEL */} * vec4(a_pos,1.0);
        UV = a_uv;
    }
`;
export const GLSL_F_BASIC = `
    #version 100
    precision mediump float;

    varying vec2 UV;

    uniform sampler2D ${"texSample" /* TEX_SAMPLE */};

    void main(void) {
        gl_FragColor = texture2D(${"texSample" /* TEX_SAMPLE */}, UV);
    }
`;
export const GLSL_V_PHONG = `
    #version 100
    precision mediump float;
    ...
    uniform mat4 ${"view" /* VIEW */};
    uniform mat4 ${"projection" /* PROJ */};
    uniform mat3 ${"normal_matrix" /* NORM_MAT */};

    void main(void) {
        gl_Position = ${"projection" /* PROJ */} * ${"view" /* VIEW */} * ${"model" /* MODEL */} * vec4(a_pos,1.0);
        UV = a_uv;
        fragPos = vec3(${"model" /* MODEL */} * vec4(a_pos, 1.0));
        normal = ${"normal_matrix" /* NORM_MAT */} * a_normals;
    }
`;
export const GLSL_F_PHONG = `
    #version 100
    ...
    uniform vec3 ${"cameraPos" /* CAM_POS */};
    ...
    void main(void) {
        ...
        for(int i = 0; i < MAX_LIGHTS; i++) {
            ...
            result += calcPhongShading(material, lights[i], fragPos, ${"cameraPos" /* CAM_POS */}, normal);
        }
        gl_FragColor = texture2D(${"texSample" /* TEX_SAMPLE */}, UV) * vec4(result, 1.0);
    }
`;
export const GLSL_V_BASIC_I = `
    #version 100
    precision mediump float;
    ...
    void main(void) {
        gl_Position = ${"projection" /* PROJ */} * ${"view" /* VIEW */} * model * vec4(a_pos,1.0);
        UV = a_uv;
    }
`;
export const GLSL_V_PHONG_I = `
    #version 100
    precision mediump float;
