...
export class WebGLUniformHandler {
    ...
    public readonly uniforms: {[name: string] : WebGLUniform};
    ...
    setUniformsFromObject(uniforms: Object) {
    ...
    setUniformsFromHandler(uHandler: WebGLUniformHandler) {
        for(const name of this.keys) {  
    ...
    set(name: string, value: number | number[] | boolean | readonly number[]) {
    ...
    private createUniformList(gl: WebGLRenderingContext) : Readonly<{[name: string] : WebGLUniform}> {
        var uniforms: {[name: string] : WebGLUniform} = {};
